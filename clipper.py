...
def format_time(delta: datetime.timedelta) -> str:
    """Format a timedelta object into SRT time format (HH:MM:SS,mmm)."""
...
def merge_subtitle_ranges(
    ...
    events = sorted(
        [
            (
                sub.start.total_seconds() - pre_buffer,
                sub.end.total_seconds() + post_buffer,
                sub,
            )
    ...
    merged = []
    current_start, current_end, current_subs = (
        events[0][0],
        events[0][1],
        [events[0][2]],
    )
    ...
    merged.append((current_start, current_end, current_subs))
    logger.debug(
        f"Merged clips: {[(s, e, [sub.content for sub in subs]) for s, e, subs in merged]}"
    )
    return merged
...
def process_video(
    ...
    target_to_keywords: Dict[str, Set[str]],
    output_dir: Path,
    ...
    ##FIXME: subtitle_file is breaking
    subtitles = list(load_subtitles_stream(str(subtitle_file)))
    matched_subtitles = find_keywords(subtitles, search_targets)
    logger.debug(f"Matched subtitles: {[sub.content for sub in matched_subtitles]}")
    ...
    for i, (start_time, end_time, clip_subs) in enumerate(
        tqdm(merged_clips, desc=f"Processing {video_file.name}")
    ):
        start_time = max(0, start_time)
        end_time = min(end_time, video_duration)
        duration = end_time - start_time
        logger.debug(f"Clip {i}: {start_time} to {end_time} ({duration}s)")

        # Filter subtitles to only those within the clip’s time range
        clip_subs_filtered = [
            sub
            for sub in subtitles
            if sub.end.total_seconds() > start_time
            and sub.start.total_seconds() < end_time
        ]
        logger.debug(
            f"Filtered subtitles for clip {i}: {[sub.content for sub in clip_subs_filtered]}"
        )
        ...
        present_targets = {
            ...
            if target.lower() in sub.content.lower()
        }
        matched_keywords = set()
        for target in present_targets:
            matched_keywords.update(target_to_keywords.get(target, set()))
        keyword_prefix = (
            "-".join(sorted(matched_keywords)) if matched_keywords else "unknown"
        )
        ...
        # Convert filtered srt.Subtitle objects to full SRT entry strings
        clip_subs_entries = [
        ...
        save_transcript(clip_subs_filtered, transcript_file)
        metadata = {
            "source_file": str(video_file),
            "keywords": list(matched_keywords),
            "start_time": start_time,
            "end_time": end_time,
            "transcript": [sub.content for sub in clip_subs_filtered],
        }
        save_metadata(metadata, metadata_file)
        logger.info(
            f"Created clip {output_file.name} with keywords {list(matched_keywords)} from {video_file} at {start_time}-{end_time}"
        )


def process_video_wrapper(
    args: Tuple[
        Path, Path, Set[str], List[str], Dict[str, Set[str]], Path, float, float, bool
    ],
    max_retries: int = 3,
) -> None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Clipper: Extract video segments based on subtitles."
    )
    parser.add_argument(
        "video_file", type=Path, nargs="?", help="Path to the video file"
    )
    parser.add_argument(
        "subtitle_file", type=Path, nargs="?", help="Path to the SRT subtitle file"
    )
    ...
    parser.add_argument(
        "--directory",
        type=Path,
        help="Source directory with videos for batch processing",
    )
    parser.add_argument("--config", type=Path, help="Path to configuration file")
    parser.add_argument(
        "--pre-buffer", type=float, default=20.0, help="Seconds before subtitle start"
    )
    parser.add_argument(
        "--post-buffer", type=float, default=20.0, help="Seconds after subtitle end"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use-subdirs",
        action="store_true",
        help="Organize outputs into subdirectories",
    )
    ...
    if args.config:
        config = ClipperConfig.from_file(args.config)
        args.video_file = (
            Path(config.video_file) if config.video_file else args.video_file
        )
        args.subtitle_file = (
            Path(config.subtitle_file) if config.subtitle_file else args.subtitle_file
        )
        args.keywords = config.keywords if config.keywords else args.keywords
        args.output_dir = (
            Path(config.output_dir) if config.output_dir else args.output_dir
        )
        ...
        pre_buffer = (
            config.pre_buffer if config.pre_buffer is not None else args.pre_buffer
        )
        post_buffer = (
            config.post_buffer if config.post_buffer is not None else args.post_buffer
        )
        max_workers = (
            config.max_workers if config.max_workers is not None else args.max_workers
        )
        use_subdirs = (
            config.use_subdirs if config.use_subdirs is not None else args.use_subdirs
        )
    ...
    search_targets, target_to_keywords = get_all_search_targets(
        keywords_set, word_alt_map or {}
    )
    ...
    if args.directory:
        if not args.directory.exists() or not args.directory.is_dir():
            ...
            return
        logger.info(
            f"Processing all videos in directory: {args.directory} with {max_workers} workers"
        )
        ...
        tasks = [
            ...
            if video_file.with_suffix(".srt").exists()
            and str(video_file) not in completed_files
        ]
        ...
        with open(progress_log, "a") as log:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_file = {
                    executor.submit(process_video_wrapper, task): task[0]
                    for task in tasks
                }
                for future in tqdm(
        ...
        if (
            not args.video_file
            or not args.subtitle_file
            or not args.video_file.exists()
            or not args.subtitle_file.exists()
        ):
            logger.error(
                "Video file or subtitle file does not exist or was not provided"
            )
            return
        process_video(
            args.video_file,
